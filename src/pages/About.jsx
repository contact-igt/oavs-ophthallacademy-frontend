...
const milestones = [
  {
    ...
    images: ["/assets/milestones/13th_mar_img1.png"],
    tag: "Chief Guest",
  },
  {
    date: "14 Mar 2026",
...
const ImageGrid = ({ images, title }) => {
  ...
  const wrapBase =
    "relative overflow-hidden shadow-md border border-gray-100 group cursor-zoom-in";
    ...
    <>
      ...
      {images.length === 2 && (
        <div className="grid grid-cols-2 gap-2">
          {images.map((src, i) =>
            renderThumb(src, i, "h-56 md:h-64 rounded-xl"),
          )}
        </div>
      )}
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {images
              .slice(1)
              .map((src, i) =>
                renderThumb(src, i + 1, "h-40 md:h-44 rounded-xl"),
              )}
          </div>
        </div>
      )}
      ...
      {images.length >= 4 && (
        <div className="grid grid-cols-2 gap-2">
          {images
            .slice(0, 4)
            .map((src, i) => renderThumb(src, i, "h-44 md:h-52 rounded-xl"))}
        </div>
      )}
    </>
  );
...
const TagBadge = ({ tag, className = "" }) => (
  <span
    className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest bg-[#F47B20] text-white px-3 py-1 rounded-full shadow-md ${className}`}
  >
...
const MilestoneContent = ({ milestone, align }) => (
  <div className={align === "right" ? "text-right" : "text-left"}>
    ...
    <div
      className={`w-10 h-[3px] bg-[#F47B20] rounded-full mb-3 ${align === "right" ? "ml-auto" : ""}`}
    />
  ...
  return (
    <>
      ...
      <div className="hidden md:grid md:grid-cols-2 relative py-14">
        ...
        {/* Left cell */}
        <div
          className={`pr-20 flex flex-col justify-center ${isEven ? "items-end" : "items-start"}`}
        >
          {isEven ? (
      ...
      <div className="md:hidden flex gap-0 pb-10">
        ...
        <div className="flex-1 min-w-0 pl-3">
          ...
          <h3 className="text-base font-bold text-[#163A5F] mb-1 leading-snug">
            {milestone.title}
          </h3>
  ...
  <div className="grid md:grid-cols-2 gap-6 mb-20 max-w-5xl mx-auto">
    ...
    <div className="relative rounded-3xl overflow-hidden shadow-xl border border-[#163A5F]/15">
      ...
      <div className="bg-[#163A5F] px-8 pt-6 pb-5">
        ...
        <h3 className="text-white text-2xl font-bold flex items-center gap-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="w-6 h-6 text-[#F47B20] flex-shrink-0"
            viewBox="0 0 24 24"
            fill="none"
            stroke="currentColor"
            strokeWidth="2.2"
        ...
        {[
          ...
          <div key={i} className="flex items-start gap-3">
            <div className="mt-1 flex-shrink-0 w-6 h-6 rounded-full bg-[#163A5F] flex items-center justify-center shadow-md">
              <span className="text-[#F47B20] text-[10px] font-black">
                {i + 1}
              </span>
            </div>
    ...
    <div className="relative rounded-3xl overflow-hidden shadow-xl border border-[#163A5F]/15">
      ...
      <div className="bg-white px-8 py-6">
        ...
        <div className="text-6xl leading-none text-[#F47B20]/20 font-serif mb-1 select-none">
          &ldquo;
        </div>
        <p className="text-gray-600 text-sm leading-relaxed -mt-4">
          Create a cadre of eye care professionals with the highest standards of
          skills, knowledge and attitude and to empower the trainees by
          providing a unique learning experience to develop them to their
          optimum potential through regular teaching, training and development.
        </p>
        {/* Keyword pills */}
        <div className="mt-5 flex flex-wrap gap-2">
          {["Skills", "Knowledge", "Attitude", "Mentorship", "Development"].map(
            (tag, i) => (
              <span
                key={i}
                className="text-xs font-semibold text-[#163A5F] bg-blue-50 border border-blue-100 px-3 py-1 rounded-full"
              >
                {tag}
              </span>
            ),
    ...
    <div className="bg-[#163A5F] px-8 py-7 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        ...
        <p className="text-blue-200 text-sm mt-1">
          Internship MOUs signed with leading hospitals across India &amp;
          abroad
        </p>
      </div>
      {/* Count badge */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {skillPartners.map((partner, i) => (
          ...
          >
            ...
            <span className="text-[#163A5F] group-hover:text-white text-sm font-medium leading-snug transition-colors duration-200">
              {partner}
            </span>
          </div>
        ))}
...
const facultyData = [
  ...
  },
  {
    ...
    description:
      "Clinical Optometrist at Sheikh Khalifa Medical City, Abu Dhabi, with 20+ years in clinical and academic optometry. Specialises in specialty contact lenses, low vision rehabilitation, and vision care for People of Determination. Active speaker at national & international conferences, pursuing a PhD and Fellowship in Vision Therapy (ASCO, UK).",
    accentColor: "from-[#163A5F] to-[#1e4d7a]",
    icon: GraduationCap,
    credentials: [
      "Sheikh Khalifa Medical City",
      "Low Vision Specialist",
      "PhD Candidate, ASCO UK",
    ],
  },
  {
  ...
  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-gray-100 flex flex-col h-full">
      ...
      <div className="p-6 pt-5 flex flex-col flex-1">
        <h3 className="text-[17px] font-bold text-[#163A5F] text-center mb-1 leading-tight">
          {faculty.name}
        </h3>
        <p className="text-[#F47B20] font-semibold text-[11px] uppercase tracking-wider text-center mb-4 leading-snug">
          {faculty.role}
        </p>
      ...
      <div className="bg-[#163A5F] text-white py-20 px-4 mb-12 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          <p className="text-blue-100 text-lg">
            Leading Optometry Training Institute in India
          </p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 pb-20">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center mb-16">
          <div className="bg-white rounded-3xl p-10 shadow-lg h-full">
            ...
            <p className="text-gray-600 mb-6 leading-relaxed">
              Ophthall Academy of Vision Sciences (OAVS) is a dedicated
              optometry training institute in India committed to transforming
              clinical education. Founded by Dr. Tamilarasan Senthil, OAVS was
              built to address the growing demand for skilled optometrists
              across hospitals and optical retail chains. This is a part of
              Ophthall Practice Development and is run by Ophthalmologists and
              senior-level leaders in Optometry.
            </p>
            <div className="bg-blue-50 p-6 rounded-xl border-l-4 border-[#F47B20]">
              ...
              <p className="text-gray-700 italic">
                "To build India's most practical and trusted optometry education
                ecosystem."
              </p>
            </div>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-20">
          <div className="bg-white p-8 rounded-2xl shadow-md border-t-4 border-[#163A5F]">
            <h3 className="text-xl font-bold text-[#163A5F] mb-4">
              The Industry Gap
            </h3>
            <p className="text-gray-600 mb-4 leading-relaxed">
              India's optometry industry is valued between ₹8,000–20,000 crore.
              However, the lack of structured clinical training creates a huge
              gap between academic qualification and job readiness.
            </p>
          </div>
        ...
        <section className="mt-24">
          ...
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 max-w-xl mx-auto text-sm leading-relaxed">
              Our faculty brings together leading ophthalmologists, senior
              optometrists, and industry practitioners committed to shaping the
              next generation of clinical professionals.
            </p>
          </div>

          {/* Responsive 3-column grid for all faculty */}
        ...
        <section className="mt-28 pb-16">
          ...
          <div className="text-center mb-14">
            ...
            <div className="w-16 h-1 bg-[#F47B20] mx-auto rounded-full mb-4" />
            <p className="text-gray-500 max-w-xl mx-auto text-sm leading-relaxed">
              A recap of our key activities, academic partnerships, and
              institutional engagements as we continue to transform optometry
              education.
            </p>
          </div>
